...
# Phenotypes: DO ovary counts before PC1 regression
# get median by pos
...
# Phenotypes: DO ovary counts after PC1 regression
...
```{r ova_dist_plot, fig.width=12, fig.height=6}
...
ovary_counts_pos_long %>% 
  ...
  facet_wrap(~ovary+position, nrow = 2, ncol = 5)
...
### Regressing out PC1 and PC2 to remove variation introduced by technical covariates. 

```{r regress_pc1}


ova_pc1_reg <- apply( ovary_counts_pos_wide, 2, function(y){
                                                lm_obj=lm(y~pca_ova_df$PC1+pca_ova_df$PC2);
                                                lm_obj$residuals}
  )
...
### Heritability estimates for some positions are very small suggesting low genetic contribution or the prescence of unresoled technical variation.
...
<!-- ### QTL map of DO ovary counts using unprocessed data -->
...
### QTL map of DO ovary counts using processed data (PC1+PC2 regressed out)


```{r qtl_peaks_plot, fig.width=12, fig.height=6, fig.cap="Lineplot showing the genomic location of suggestive QTL peaks for all phenotypes with LOD scores annotated. Genetic mapping is done using generation of DO animals and the date of submission to histology as additive covariates with ovary counts from 133 animals in total."}
...
for( i in 1:nrow(qtl_peaks_lod6)){
  ...
  pc <- ova_pc1_reg[,qtl_peaks_lod6$lodcolumn[i]]
